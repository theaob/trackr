...
> = {
  ...
  IN_PROGRESS: {
    ...
    border: "border-jira-blue/40 hover:border-jira-blue",
    badgeBg: "bg-blue-100",
    badgeText: "text-jira-blue",
    label: "In Progress",
    accent: "#0052CC",
  },
  DONE: {
...
export default function WorkflowGraphView({
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-50/50 rounded-lg border border-jira-gray-200 overflow-hidden select-none">
      {/* Top Toolbar */}
      <div className="h-11 px-4 border-b border-jira-gray-200 bg-white flex items-center justify-between gap-3 shrink-0 z-20">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 text-xs text-jira-gray-600">
            <GitBranch className="w-3.5 h-3.5 text-jira-blue" />
            <span className="font-bold text-jira-navy">Workflow Graph</span>
            <span className="text-[11px] text-jira-gray-400">
              ({statuses.length} statuses, {transitions.length} transitions)
            </span>
          </div>

          {connectingFromId && (
            <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-jira-blue-light border border-jira-blue/40 text-jira-blue text-xs font-semibold animate-pulse">
              <span>Connect to destination status...</span>
              <button
                type="button"
                onClick={() => setConnectingFromId(null)}
                className="p-0.5 hover:bg-jira-blue/20 rounded"
                title="Cancel (Esc)"
              >
                <X className="w-3 h-3" />
              </button>
        ...
        <div className="flex items-center gap-2">
          {/* Zoom controls */}
          <div className="flex items-center border border-jira-gray-300 rounded bg-white overflow-hidden text-xs">
            <button
              type="button"
              onClick={handleZoomOut}
              className="p-1.5 hover:bg-jira-gray-100 text-jira-gray-600 transition-colors"
              title="Zoom Out"
            >
            ...
            <button
              ...
              className="px-2 py-1 hover:bg-jira-gray-100 text-jira-gray-700 font-semibold border-x border-jira-gray-200 text-[11px]"
              title="Reset Zoom"
            >
              {Math.round(zoom * 100)}%
            </button>
          ...
          <button
            ...
            className="px-2.5 py-1 text-xs font-semibold text-jira-gray-700 bg-white border border-jira-gray-300 hover:bg-jira-gray-100 rounded flex items-center gap-1.5 transition-colors shadow-2xs"
          ...
          {onAddStatusClick && canManage && (
            <button
              ...
              className="px-2.5 py-1 text-xs font-semibold text-white bg-jira-blue hover:bg-jira-blue-hover rounded flex items-center gap-1 transition-colors shadow-2xs"
            >
          ...
          <div className="absolute inset-0 pointer-events-none flex" style={{ gap: `${COL_WIDTH - NODE_WIDTH}px`, paddingLeft: `${START_X}px`, paddingTop: "20px" }}>
            {(["TODO", "IN_PROGRESS", "DONE"] as WorkflowStatusCategory[]).map((cat, idx) => (
              <div
                ...
                className="h-full border-t-2 border-dashed border-jira-gray-200/90 pt-1"
              >
                <span className="text-[10px] font-bold uppercase tracking-wider text-jira-gray-400">
            ...
            return (
              <div
                ...
                className={`absolute rounded-lg border-2 bg-white shadow-xs transition-shadow cursor-move z-20 select-none ${
                  ...
                  isSelected ? "ring-2 ring-jira-blue ring-offset-2 border-jira-blue shadow-md" : ""
                } ${
              ...
              >
                {/* Node Card Header */}
                <div
                  className={`h-7 px-2.5 rounded-t-md flex items-center justify-between border-b border-jira-gray-200 ${style.headerBg}`}
                >
                ...
                <div className="p-2 flex flex-col justify-between h-[52px]">
                  <div className="flex items-center justify-between gap-1">
                    <h4 className="text-xs font-bold text-jira-navy truncate" title={status.name}>
                      {prettifyStatusName(status.name)}
                    </h4>
                    {status.wipLimit !== null && (
                      <span className="text-[10px] font-semibold text-jira-gray-500 bg-jira-gray-100 px-1 rounded">
                        WIP: {status.wipLimit}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between text-[10px] text-jira-gray-500 pt-0.5">
                    ...
                    {canManage && (
                      <button
                        ...
                        className="text-jira-blue hover:text-jira-blue-hover hover:underline font-semibold flex items-center gap-0.5"
                ...
                {canManage && (
                  <div
                    ...
                    className="absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-white border-2 border-jira-blue hover:bg-jira-blue text-jira-blue hover:text-white flex items-center justify-center cursor-crosshair shadow-sm transition-colors z-30 group"
      ...
      {selectedStatus && (
        <div className="border-t border-jira-gray-200 bg-white p-4 shrink-0 z-30 animate-in slide-in-from-bottom-2 duration-150">
          <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-jira-gray-200">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-bold text-jira-navy">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="p-1 text-jira-gray-400 hover:text-jira-navy rounded"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
          ...
          <div className="pt-3 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            ...
            <div>
              <div className="text-[11px] font-bold text-jira-gray-600 uppercase tracking-wider mb-2 flex items-center gap-1">
                <ArrowRight className="w-3 h-3 text-jira-blue" />
                Can transition TO:
              </div>
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1 rounded-full text-xs font-medium border flex items-center gap-1.5 transition-colors ${
                          isAllowed
                            ...
                            : "bg-white text-jira-gray-600 border-jira-gray-300 hover:border-jira-blue hover:text-jira-blue"
                        } disabled:opacity-50`}
                      >
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1 rounded-full text-xs font-medium border flex items-center gap-1.5 transition-colors ${
                          isAllowed
                            ...
                            : "bg-white text-jira-gray-600 border-jira-gray-300 hover:border-emerald-600 hover:text-emerald-700"
                        } disabled:opacity-50`}
                      >
                        {isAllowed && <Check className="w-3 h-3" />}
      ...
      <div className="px-4 py-2 bg-slate-50 border-t border-jira-gray-200 flex flex-wrap items-center justify-between text-[11px] text-jira-gray-500 gap-2 shrink-0">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-jira-blue" />
            In Progress
          </span>
          <span className="flex items-center gap-1">
        ...
        <span className="text-jira-gray-400 hidden sm:inline">Drag cards to arrange &bull; Click card to inspect</span>
      </div>
    </div>
  );
}
